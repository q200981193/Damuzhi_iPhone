...
@property (retain) NSString* guideId;
...
- (void) writeToCodedOutputStream:(PBCodedOutputStream*) output {
  if (self.hasGuideId) {
    [output writeString:1 value:self.guideId];
  }
  if (self.hasCityId) {
...
- (int32_t) serializedSize {
  ...
  size = 0;
  if (self.hasGuideId) {
    size += computeStringSize(1, self.guideId);
  }
  if (self.hasCityId) {
...
- (CommonTravelGuide_Builder*) mergeFromCodedInputStream:(PBCodedInputStream*) input extensionRegistry:(PBExtensionRegistry*) extensionRegistry {
  ...
  while (YES) {
    ...
    switch (tag) {
      ...
      case 10: {
        [self setGuideId:[input readString]];
        break;
      }
      case 18: {
...
- (int32_t) serializedSize {
  ...
  if (self.hasCityId) {
    size += computeStringSize(3, self.cityId);
  }
  size += self.unknownFields.serializedSize;
  memoizedSerializedSize = size;
  return size;
